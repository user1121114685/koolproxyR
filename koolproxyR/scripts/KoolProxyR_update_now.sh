...
update_kpr(){
    ...
    if [[ "$koolproxyR_installing_version" != "$koolproxyR_version_now" ]]; then
        ...
        for i in {1..5}; do
            ...
            wget -4 -a /tmp/upload/kpr_log.txt -O /tmp/upload/koolproxyR.tar.gz https://dev.tencent.com/u/shaoxia1991/p/koolproxyr/git/raw/master/koolproxyR.tar.gz
            ...
            if [[ "$koolproxyR_installing_md5" == "$koolproxyR_download_md5" ]]; then
                wget -4 -a /tmp/upload/kpr_log.txt -O /koolshare/scripts/kpr_tar_install.sh https://dev.tencent.com/u/shaoxia1991/p/koolproxyr/git/raw/master/kpr_tar_install.sh && chmod 777 /koolshare/scripts/kpr_tar_install.sh
